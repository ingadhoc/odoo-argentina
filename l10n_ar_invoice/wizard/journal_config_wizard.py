...
class account_journal_document_config(osv.osv_memory):
    ...
    def confirm(self, cr, uid, ids, context=None):
        ...
        self.create_journals(
            cr, uid, wizard.debit_notes, wizard.credit_notes, journal_ids, context=context)
        ...
        for journal in self.pool['account.journal'].browse(cr, uid, journal_ids, context=context):
            ...
            if journal_type == 'sale':
                ...
                self.create_journal_document(
                    cr, uid, letter_ids, document_type, journal.id, credit_notes, debit_notes, context)
    ...
    def create_journal_document(self, cr, uid, letter_ids, document_type, journal_id, credit_notes, debit_notes, context=None):
        ...
        document_class_ids = document_class_obj.search(cr, uid, [(
            'document_letter_id', 'in', letter_ids), ('document_type', '=', document_type)], context=context)
            ...
            if journal.type == 'sale':
                if document_type == 'invoice' or document_type == 'debit_note' and debit_notes == 'own_sequence':
                    sequence_id = self.create_sequence(
                        cr, uid, document_class.name, journal, context)
                    ...
                    if journal_docuent_ids:
                        sequence_id = journal_document_obj.browse(
                            cr, uid, journal_docuent_ids[0], context=context).sequence_id.id
