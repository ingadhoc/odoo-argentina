...
class TestPadronSantaFeContributorType(TestArCommon):
    ...
    def test_padron_data_threads_the_base_down_to_the_tax(self):
        ...
        with self._without_demo_user():
            cm_aliquot, __, cm_base = self.wth_line._get_padron_data(
                self._create_partner(CUIT_MULTILATERAL), PADRON_FROM, PADRON_TO
            )
            local_aliquot, __, local_base = self.wth_line._get_padron_data(
                self._create_partner(CUIT_LOCAL), PADRON_FROM, PADRON_TO
            )
            missing_aliquot, __, missing_base = self.wth_line._get_padron_data(
                self._create_partner(CUIT_NOT_IN_PADRON), PADRON_FROM, PADRON_TO
            )

        self.assertEqual((cm_aliquot, cm_base), (0.6, "iibb_total"))
        self.assertEqual((local_aliquot, local_base), (0.8, False))
        self.assertEqual((missing_aliquot, missing_base), (None, False))

        cm_tax = self.wth_line._ensure_tax(cm_aliquot, l10n_ar_tax_type=cm_base)
