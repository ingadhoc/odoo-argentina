...
class sale_order_line(models.Model):
    ...
    printed_price_unit = fields.Monetary(
        compute="_printed_prices",
        string='Unit Price'
    )
    printed_price_net = fields.Monetary(
        compute="_printed_prices",
        string='Net Price',
    )
    printed_price_subtotal = fields.Monetary(
        compute="_printed_prices",
        string='Subtotal',
    )
...
class sale_order(models.Model):
    ...
    # no podemos usar onchange porque ya hay onchange de api vieja sobre
    # partner'
    @api.one
    @api.depends(
        'partner_id',
    ...
    def _printed_prices(self):
        ...
        if self.vat_discriminated:
            ...
            printed_tax_ids = []
            # printed_tax_ids = [
            # x.id for x in self.tax_line if x.tax_code_id.parent_id.name !=
            # 'IVA']
    ...
    printed_amount_tax = fields.Monetary(
        compute="_printed_prices",
        string='Tax'
    )
    printed_amount_untaxed = fields.Monetary(
        compute="_printed_prices",
        string='Subtotal'
    )
    exempt_amount = fields.Monetary(
